   // Declaration of leaf types
   /* Int_t         */  runNumber = 0;
   /* Int_t         */  eventNumber = 0;
   /* Int_t         */  channelNumber = 0;
   /* Int_t         */  lbNumber = 0;
   /* Int_t         */  rndRunNumber = 0;
   /* Int_t         */  dataPeriod = 0;
   /* Float_t       */  mu = 0;
   /* Float_t       */  mcWeight = 0;
   /* Float_t       */  eventWeight_PRETAG = 0;
   /* Float_t       */  eventWeight_BTAG = 0;
   /* Int_t         */  pvxp_n = 0;
   /* Int_t         */  hfor = 0;
   /* Float_t       */  vxp_z = 0;
   /* Float_t       */  mc_vxp_z = 0;
   /* Double_t      */  mcevt_pdf1 = 0;
   /* Double_t      */  mcevt_pdf2 = 0;
   /* Int_t         */  mcevt_pdf_id1 = 0;
   /* Int_t         */  mcevt_pdf_id2 = 0;
   /* Double_t      */  mcevt_pdf_scale = 0;
   /* Double_t      */  mcevt_pdf_x1 = 0;
   /* Double_t      */  mcevt_pdf_x2 = 0;
   /* Float_t       */  scaleFactor_PILEUP = 0;
   /* Float_t       */  scaleFactor_ELE = 0;
   /* Float_t       */  scaleFactor_MUON = 0;
   /* Float_t       */  scaleFactor_BTAG = 0;
   /* Float_t       */  scaleFactor_TRIGGER = 0;
   /* Float_t       */  scaleFactor_WJETSNORM = 0;
   /* Float_t       */  scaleFactor_WJETSSHAPE = 0;
   /* Float_t       */  scaleFactor_JVFSF = 0;
   /* Float_t       */  scaleFactor_ZVERTEX = 0;
   /* Float_t       */  scaleFactor_ALLPRETAG = 0;
   /* Float_t       */  scaleFactor_ALLBTAG = 0;
   /* Int_t         */  truE = 0;
   /* Int_t         */  truM = 0;
   /* Bool_t        */  trigE = 0;
   /* Bool_t        */  trigM = 0;
   /* Bool_t        */  passGRL = 0;
   /* Bool_t        */  cosmicEvent = 0;
   /* Bool_t        */  isOS = 0;
   /* Bool_t        */  hasGoodVertex = 0;
   /* Float_t       */  ht = 0;
   /* Float_t       */  mass = 0;
   /* Float_t       */  massInv_LL = 0;
   /* Float_t       */  massInv_BQQ = 0;
   /* Float_t       */  massTrans_BLMet = 0;
   /* Float_t       */  massTrans_LMet = 0;
   /* UInt_t        */  flag_DL = 0;
   /* UInt_t        */  flag_TTZ = 0;
   /* UInt_t        */  channel_DL = 0;
   /* UInt_t        */  channel_TTZ = 0;
   /* Float_t       */  scaledWeight = 0;
   /* UInt_t        */  lep_n = 0;
   for (int il=0; il<4; il++) {
   /* Bool_t        */  lep_truthMatched[il] = 0;   //[lep_n]
   /* UShort_t      */  lep_trigMatched[il] = 0;   //[lep_n]
   /* Float_t       */  lep_pt[il] = 0;   //[lep_n]
   /* Float_t       */  lep_eta[il] = 0;   //[lep_n]
   /* Float_t       */  lep_phi[il] = 0;   //[lep_n]
   /* Float_t       */  lep_E[il] = 0;   //[lep_n]
   /* Float_t       */  lep_z0[il] = 0;   //[lep_n]
   /* Float_t       */  lep_charge[il] = 0;   //[lep_n]
   /* Bool_t        */  lep_isTight[il] = 0;   //[lep_n]
   /* UInt_t        */  lep_type[il] = 0;   //[lep_n]
   /* UInt_t        */  lep_flag[il] = 0;   //[lep_n]
   /* Float_t       */  el_cl_eta[il] = 0;   //[lep_n]
   /* Float_t       */  lep_ptcone30[il] = 0;   //[lep_n]
   /* Float_t       */  lep_etcone20[il] = 0;   //[lep_n]
   /* Float_t       */  lep_miniIso10_4[il] = 0;   //[lep_n]
   /* Float_t       */  lep_trackd0pvunbiased[il] = 0;   //[lep_n]
   /* Float_t       */  lep_tracksigd0pvunbiased[il] = 0;   //[lep_n]
   /* Float_t       */  massTrans_LMet_Vec[il] = 0;   //[lep_n]
   /* Int_t         */  lep_LHflag[il] = 0;   //[lep_n]
   /* Int_t         */  lep_ppflag[il] = 0;   //[lep_n]
   }
   /* UInt_t        */  lepPair_n = 0;
   for (int ilp=0; ilp<6; ilp++) {
   /* Bool_t        */  isSameFlavor_LL_Vec[ilp] = 0;   //[lepPair_n]
   /* Bool_t        */  isOppSign_LL_Vec[ilp] = 0;   //[lepPair_n]
   /* Float_t       */  massInv_LL_Vec[ilp] = 0;   //[lepPair_n]
   }
   /* Float_t       */  met_sumet = 0;
   /* Float_t       */  met_et = 0;
   /* Float_t       */  met_phi = 0;
   /* UInt_t        */  jet_n = 0;
   /* UInt_t        */  bjet_n = 0;
   /* UInt_t        */  alljet_n = 0;
   for (int ij=0; ij<12; ij++) {
   /* Float_t       */  jet_pt[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_eta[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_phi[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_E[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_m[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_jvf[ij] = 0;   //[alljet_n]
   /* Int_t         */  jet_trueflav[ij] = 0;   //[alljet_n]
   /* Int_t         */  jet_truthMatched[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_SV0[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_MV1[ij] = 0;   //[alljet_n]
   /* Float_t       */  jet_MV1c[ij] = 0;   //[alljet_n]
   /* UInt_t        */  jet_flag[ij] = 0;   //[alljet_n]
   }
   /* Int_t         */  ttHDiLepBlind = 0;
   /* vector<int>   */  el_topgood = 0;
   /* Bool_t        */  EF_2e12Tvh_loose1 = 0;
   /* Bool_t        */  EF_e24vh_medium1_e7_medium1 = 0;
   /* Bool_t        */  EF_e12Tvh_medium1_mu8 = 0;
   /* Bool_t        */  EF_e24vhi_loose1_mu8 = 0;
   /* Bool_t        */  EF_e24vhi_medium1 = 0;
   /* Bool_t        */  EF_e60_medium1 = 0;
   /* Bool_t        */  EF_mu24i_tight = 0;
   /* Bool_t        */  EF_mu36_tight = 0;
   /* Bool_t        */  EF_mu18_tight_mu8_EFFS = 0;
   /* Float_t       */  met_etx = 0;
   /* Float_t       */  met_ety = 0;
   /* Float_t       */  met_RefEle_sumet = 0;
   /* Float_t       */  met_RefEle_etx = 0;
   /* Float_t       */  met_RefEle_ety = 0;
   /* Float_t       */  met_RefJet_sumet = 0;
   /* Float_t       */  met_RefJet_etx = 0;
   /* Float_t       */  met_RefJet_ety = 0;
   /* Float_t       */  met_SoftJets_sumet = 0;
   /* Float_t       */  met_SoftJets_etx = 0;
   /* Float_t       */  met_SoftJets_ety = 0;
   /* Float_t       */  met_CellOut_sumet = 0;
   /* Float_t       */  met_CellOut_etx = 0;
   /* Float_t       */  met_CellOut_ety = 0;
   /* Float_t       */  met_Muon_Total_Muid_etx = 0;
   /* Float_t       */  met_Muon_Total_Muid_ety = 0;
   /* UInt_t        */  particle_jet_n = 0;
   /* Float_t       */  particle_jet_hthad = 0;
   /* Float_t       */  particle_jet_lead_pt = 0;
   /* UInt_t        */  mc_n = 0;
   /* vector<float>  		*/  mc_E = 0;
   /* vector<float>  		*/  mc_pt = 0;
   /* vector<float>  	      	*/  mc_eta = 0;
   /* vector<float>  	      	*/  mc_phi = 0;
   /* vector<int>    	      	*/  mc_barcode = 0;
   /* vector<int>    	      	*/  mc_status = 0;
   /* vector<int>    	      	*/  mc_pdgId = 0;
   /* vector<float>  	      	*/  mc_charge = 0;
   /* vector<vector<int> >    	*/  mc_parent_index = 0;
   /* vector<vector<int> >    	*/  mc_child_index = 0;
   /* UInt_t        */  true_lep_n = 0;
   /* vector<float> 		*/  true_lep_E = 0;
   /* vector<float>   		*/  true_lep_pt = 0;
   /* vector<float>             */  true_lep_eta = 0;
   /* vector<float>             */  true_lep_phi = 0;
   /* vector<unsigned int> 	*/  true_lep_type = 0;
   /* vector<int> 		*/  true_lep_charge = 0;
   /* UInt_t        */  true_jet_n = 0;
   /* vector<float>             */  true_jet_E = 0;
   /* vector<float>             */  true_jet_pt = 0;
   /* vector<float>             */  true_jet_eta = 0;
   /* vector<float>             */  true_jet_phi = 0;
   /* vector<float>             */  true_jet_m = 0;
   /* vector<float>             */  true_jet_dR_Bhadron = 0;
   /* UInt_t        */  true_non_int_n = 0;
   /* vector<float>             */  true_non_int_E = 0;
   /* vector<float>             */  true_non_int_pt = 0;
   /* vector<float>             */  true_non_int_eta = 0;
   /* vector<float>             */  true_non_int_phi = 0;
   /* vector<int>    		*/  true_non_int_type = 0;

