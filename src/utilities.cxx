...
namespace LIP {
	...
	void defineNumThreads (void) {
		...
		if (num != NULL) {
			number_of_threads = atoi(num);
			cout << "\033[0;32mRuning analysis with " << number_of_threads;
			cout << " threads\033[0m" << endl << endl;
		} else {
			number_of_threads = DEFAULT_THREADS;
			cout << "\033[0;31mNumber of threads not defined!" << endl;
			cout << "Running analysis for the default value of ";
			cout << number_of_threads << " threads\033[0m" << endl << endl;
		}
	...
	long long int startTimer (void) {
		char *flag = getenv("MEASURE_LIP_APP");
		long long int time;
		timeval t;
